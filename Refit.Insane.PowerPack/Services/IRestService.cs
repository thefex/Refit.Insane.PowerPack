...
namespace Refit.Insane.PowerPack.Services
{
	public interface IRestService
	{
		...
		/// <param name="cacheBehaviour">Control cache behaviour (read enum descriptions). Useful in scenario when you want to optionally force refresh - typical use-case: pull to refresh in Mobile App.</param>
		/// <typeparam name="TApi">Refit API interface</typeparam>
		/// <typeparam name="TResult">Method result type.</typeparam>
		/// <returns></returns>
		Task<Response<TResult>> Execute<TApi, TResult>(Expression<Func<TApi, Task<TResult>>> executeApiMethod, RefitCacheBehaviour cacheBehaviour = RefitCacheBehaviour.Default);
		...
		/// Delegate to control cache behaviour - (you can control it so your API could be called - even if the value is already in cache)
		...
		/// <typeparam name="TResult">Method result type</typeparam>
		/// <returns></returns>
		Task<Response<TResult>> Execute<TApi, TResult>(Expression<Func<TApi, Task<TResult>>> executeApiMethod, Func<TimeSpan?, RefitCacheBehaviour> controlCacheBehaviourBasedOnTimeSpanBetweenLastCacheUpdate);
